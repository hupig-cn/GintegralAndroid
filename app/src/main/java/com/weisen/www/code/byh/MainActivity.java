...
import com.weisen.www.code.byh.zbar.CaptureActivity;
...
//            goTo("http://"+sp.getString("server", "")+"/"+sp.getString("project", "")+"/"+sp.getString("modular", ""));
            goTo("http://"+sp.getString("server", "")+":"+sp.getString("projectOne", ""));
        }else if (id == R.id.business){
            goTo("http://"+sp.getString("server", "")+":"+sp.getString("projectTwo", ""));
        }else if (id == R.id.extension){
            goTo("http://"+sp.getString("server", "")+":"+sp.getString("projectThree", ""));
            ...
            try{
                if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION)!= PackageManager.PERMISSION_GRANTED) {
                    ...
                    message("系统信息", "服务器：" + sp.getString("server", "") +
                            "\n项目1端口：" + sp.getString("projectOne", "") +
                            "\n项目2端口：" + sp.getString("projectTwo", "") +
                            "\n项目3端口：" + sp.getString("projectThree", "") +
                ...
                .setNegativeButton("确认", new DialogInterface.OnClickListener(){
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (inputServer.getText().toString().equals("135498")){
                            ...
                            ((EditText) view.findViewById(R.id.projectIdOne)).setText(sp.getString("projectOne", ""));
                            ((EditText) view.findViewById(R.id.projectIdTwo)).setText(sp.getString("projectTwo", ""));
                            ((EditText) view.findViewById(R.id.projectIdThree)).setText(sp.getString("projectThree", ""));
                            new AlertDialog.Builder(MainActivity.this)
                                    ...
                                    .setNegativeButton("保存", new DialogInterface.OnClickListener(){
                                        ...
                                        public void onClick(DialogInterface dialog, int which) {
                                            ...
                                            editor.putString("projectOne", ((EditText)view.findViewById(R.id.projectIdOne)).getText().toString());
                                            editor.putString("projectTwo", ((EditText)view.findViewById(R.id.projectIdTwo)).getText().toString());
                                            editor.putString("projectThree", ((EditText)view.findViewById(R.id.projectIdThree)).getText().toString());
                                            editor.commit();
//                                            goTo("http://"+sp.getString("server", "")+"/"+sp.getString("project", "")+"/"+sp.getString("modular", ""));
                                            goTo("http://"+sp.getString("server", "")+":"+sp.getString("projectOne", ""));
                                        }
    ...
    public void init() {
        ...
        webSettings.setUserAgentString(ua+" Weisen/2.1 (yjf; 1.0)");
                ...
                if (resultCode == RESULT_OK) {
                    if (data != null) {
                        ...
                        if (result.toLowerCase().contains("wxp://")){
                            message("提示","不支持微信支付码。");
                            return;
                        }else if (result.toLowerCase().contains("https://qr.alipay.com/")){
                            message("提示","当前二维码为<支付宝收款码>,将跳转到支付宝客户端进行支付。");
                        }
